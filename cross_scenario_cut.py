...
def x_solve_int_BD_cut(dummy_item_index, rho, global_vars, BD_cut, UB, LB, quad_penalty=True, relax_option=True, penalty_mag=1e5,
    ...
    if len(BD_cut.keys()) > 0:
        ...
        for ite_num in [max_ite]:  # BD_cut.keys(), only add the newest cut
            ...
            prob.addConstrs(BD_cut[ite_num][i][0] + gp.quicksum(
                BD_cut[ite_num][i][1][xi_ind] * (xCi[global_var_const["indices"][0][xi_ind]] - x_hat[xi_ind]) for xi_ind
                in
                range(x_hat.size))
                            + gp.quicksum(
                BD_cut[ite_num][i][2][ri_ind] * (rCi[global_var_const["indices"][1][ri_ind]] - r_hat[ri_ind]) for ri_ind
                in
                range(r_hat.size))
                            + BD_cut[ite_num][i][3] * (eta[i] - eta_hat[gbv.item_list.index(i)]) <= 0 for i in
                            gbv.item_list
                            if (i in BD_cut[ite_num].keys()) and (i != item_ind))
